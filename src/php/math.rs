...
use crate::engine::operators::{zval_get_double, zval_get_long};
...
pub fn math_ceil(args: &[Val]) -> Result<Val, String> {
    ...
    Ok(Val::new(
        PhpValue::Double(zval_get_double(&args[0]).ceil()),
        PhpType::Double,
    ))
}
...
pub fn math_floor(args: &[Val]) -> Result<Val, String> {
    ...
    Ok(Val::new(
        PhpValue::Double(zval_get_double(&args[0]).floor()),
        PhpType::Double,
    ))
}
...
pub fn math_round(args: &[Val]) -> Result<Val, String> {
    ...
    if precision == 0 {
        ...
        Ok(Val::new(
            PhpValue::Double((value * multiplier).round() / multiplier),
            PhpType::Double,
        ))
    }
}
...
pub fn math_exp(args: &[Val]) -> Result<Val, String> {
    ...
    Ok(Val::new(
        PhpValue::Double(E.powf(zval_get_double(&args[0]))),
        PhpType::Double,
    ))
}
...
pub fn math_sin(args: &[Val]) -> Result<Val, String> {
    ...
    Ok(Val::new(
        PhpValue::Double(zval_get_double(&args[0]).sin()),
        PhpType::Double,
    ))
}
...
pub fn math_cos(args: &[Val]) -> Result<Val, String> {
    ...
    Ok(Val::new(
        PhpValue::Double(zval_get_double(&args[0]).cos()),
        PhpType::Double,
    ))
}
...
pub fn math_tan(args: &[Val]) -> Result<Val, String> {
    ...
    Ok(Val::new(
        PhpValue::Double(zval_get_double(&args[0]).tan()),
        PhpType::Double,
    ))
}
...
pub fn math_asin(args: &[Val]) -> Result<Val, String> {
    ...
    Ok(Val::new(
        PhpValue::Double(zval_get_double(&args[0]).asin()),
        PhpType::Double,
    ))
}
...
pub fn math_acos(args: &[Val]) -> Result<Val, String> {
    ...
    Ok(Val::new(
        PhpValue::Double(zval_get_double(&args[0]).acos()),
        PhpType::Double,
    ))
}
...
pub fn math_atan(args: &[Val]) -> Result<Val, String> {
    ...
    Ok(Val::new(
        PhpValue::Double(zval_get_double(&args[0]).atan()),
        PhpType::Double,
    ))
}
...
pub fn math_rand(args: &[Val]) -> Result<Val, String> {
    ...
    let random = (seed.wrapping_mul(1103515245).wrapping_add(12345) / 65536) % 32768;
    ...
    if args.len() >= 2 {
        ...
        Ok(Val::new(
            PhpValue::Long(min + (random % range) as i64),
            PhpType::Long,
        ))
    } else {
...
mod tests {
    ...
    fn test_abs() {
        assert_eq!(
            zval_get_long(&math_abs(&[Val::new(PhpValue::Long(-5), PhpType::Long)]).unwrap()),
            5
        );
        assert_eq!(
            zval_get_long(&math_abs(&[Val::new(PhpValue::Long(5), PhpType::Long)]).unwrap()),
        ...
        assert!(
            (zval_get_double(
                &math_abs(&[Val::new(
                    PhpValue::Double(-std::f64::consts::PI),
                    PhpType::Double
                )])
                .unwrap()
            ) - std::f64::consts::PI)
    ...
    fn test_round() {
        ...
        assert_eq!(
            zval_get_double(
                &math_round(&[Val::new(PhpValue::Double(3.56), PhpType::Double)]).unwrap()
            ),
            4.0
        );
    }
    ...
    fn test_sqrt() {
        assert_eq!(
            zval_get_double(&math_sqrt(&[Val::new(PhpValue::Long(9), PhpType::Long)]).unwrap()),
            3.0
        );
        assert_eq!(
            zval_get_double(&math_sqrt(&[Val::new(PhpValue::Long(16), PhpType::Long)]).unwrap()),
            4.0
        );
    }
    ...
    fn test_trig() {
        ...
        assert!(
            (zval_get_double(
                &math_sin(&[Val::new(PhpValue::Double(pi / 2.0), PhpType::Double)]).unwrap()
            ) - 1.0)
                .abs()
                < 0.001
        );
        assert!(
            (zval_get_double(
                &math_cos(&[Val::new(PhpValue::Double(0.0), PhpType::Double)]).unwrap()
            ) - 1.0)
                .abs()
                < 0.001
        );
    ...
    fn test_exp_log_log10() {
        assert!(
            (zval_get_double(
                &math_exp(&[Val::new(PhpValue::Double(1.0), PhpType::Double)]).unwrap()
            ) - std::f64::consts::E)
                .abs()
                < 1e-9
        );
        assert!(
            (zval_get_double(
                &math_log(&[Val::new(
                    PhpValue::Double(std::f64::consts::E),
                    PhpType::Double
                )])
                .unwrap()
            ) - 1.0)
        ...
        assert!(
            (zval_get_double(
                &math_log10(&[Val::new(PhpValue::Double(100.0), PhpType::Double)]).unwrap()
            ) - 2.0)
                .abs()
                < 1e-9
        );
    ...
    fn test_tan_atan2_atan_asin_acos() {
        assert!(
            (zval_get_double(
                &math_tan(&[Val::new(PhpValue::Double(0.0), PhpType::Double)]).unwrap()
            ) - 0.0)
                .abs()
                < 1e-9
        );
        assert!(
            (zval_get_double(
                &math_atan2(&[
                    Val::new(PhpValue::Double(1.0), PhpType::Double),
                    Val::new(PhpValue::Double(1.0), PhpType::Double)
                ])
                .unwrap()
        ...
        assert!(
            (zval_get_double(
                &math_atan(&[Val::new(PhpValue::Double(1.0), PhpType::Double)]).unwrap()
            ) - std::f64::consts::FRAC_PI_4)
                .abs()
                < 1e-9
        );
        assert!(
            (zval_get_double(
                &math_asin(&[Val::new(PhpValue::Double(1.0), PhpType::Double)]).unwrap()
        ...
        assert!(
            (zval_get_double(
                &math_acos(&[Val::new(PhpValue::Double(1.0), PhpType::Double)]).unwrap()
            ) - 0.0)
                .abs()
                < 1e-9
        );
